...
const updateReservationController = async ({ status, id_reserva }) => {
    ...
    try {
       ...
       const actualizacion = await reserva.update({ estatus: status }, { fields: ["estatus"] });

       if(actualizacion.dataValues.estatus === 'Success') return "Reserva Aceptada"

       if(actualizacion.dataValues.estatus === 'Reject') return "Reserva Rechazada"
    } catch (error) {
        throw Error(error.message);
    }
};
...
module.exports = { postReservationController, getAllReservationController, getReservationsAlojamientoController, updateReservationController };
