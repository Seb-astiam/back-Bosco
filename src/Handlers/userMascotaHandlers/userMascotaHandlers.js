const {createUserMascotaController, getAllUserMascotasController, deleteUserMascotaController,updateUserMascotaController} = require ('../../Controllers/userMascotaControllers/userMascotaControllers')
...
const createUserMascotaHandler = async (req, res) => {
  ...
  const getAllUserMascotasHandler= async(req,res) => {
      ...
      try {
        ...
        res.status(500).json({ error: "Error al obtener UserMascota: " + error.message });
      }
    };
    ...
    const deleteUserMascotaHandler = async (req, res) => {
      ...
      try {
        ...
        res.status(200).json({ message: 'UserMascota eliminado correctamente', mascota: mascotaEliminada });
      } catch (error) {
        res.status(500).json({ error: 'Error eliminando el UserMascota: ' + error.message });
      }
    };
    ...
    const updateUserMascotaHandler = async (req, res) => {
      ...
      try {
        ...
        res.status(200).json({ message: 'UserMascota actualizado correctamente', mascota: mascotaActualizada });
      } catch (error) {
        res.status(500).json({ error: 'Error actualizando UserMascota: ' + error.message });
      }
    };
